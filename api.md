...
任 | `dogTag` | `` | `4` | キャラクターに振られる連番です。任意の整数で指定します
...
任 | `url` | `` | `http://charasheet.vampire-blood.net/1352909` |　キャラクターの参照 URL です
任 | `info` | `` | `今麻痺しています。カードは後4枚残しています` | キャラクターのメモ欄に掲載する情報です
...
任 | `counters` | `` | `HP:32,MP:24,*poison:,*cat:1` | イニシアティブ表に表示されるキャラクター情報です。項目ごとにカンマ区切り、項目の名前と値をコロンで区切ります
任 | `statusAlias` | `` | `poison:毒,cat:猫化` | どどんとふでは counters の key の先頭に * を入力した場合、その項目が bool 値になります。この場合の表示を置き換えるために用いられます。入力形式は counters と同様です
